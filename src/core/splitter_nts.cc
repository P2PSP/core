...
namespace p2psp {
  ...
  Splitter_NTS::Splitter_NTS() : Splitter_LRS(),
    ...
    this->check_timeout_thread_ =
      std::thread(&Splitter_NTS::CheckTimeoutThread, this);

    // The thread listens to this->extra_socket_ and reports source ports
    this->listen_extra_socket_thread_ =
      std::thread(&Splitter_NTS::ListenExtraSocketThread, this);
  ...
  std::string Splitter_NTS::GenerateId() {
    ...
    for (int i = 0; i < Common_NTS::kPeerIdLength; i++) {
  ...
  void Splitter_NTS::SendTheListOfPeers(
    ...
    Common_NTS::Write<uint16_t>(msg_str,
			       std::min((unsigned int) this->number_of_monitors_,
  ...
  void Splitter_NTS::CheckIncorporatingPeerTime() {
    ...
    for (const auto& peer_iter : this->incorporating_peers_) {
	  ...
	  > Common_NTS::kMaxTotalIncorporationTime) {
	peers_to_remove.push_back(peer_id);
      }
    }
    // Actually remove the peers
  ...
  void Splitter_NTS::ListenExtraSocketThread() {
    ...
    std::vector<char> message_bytes(Common_NTS::kPeerIdLength);
    std::string message;
	...
	continue;
      ...
      if (message.size() == Common_NTS::kPeerIdLength) {
	// Send acknowledge
	try {
	...
	std::string peer_id(message.data(), Common_NTS::kPeerIdLength);
  ...
  void Splitter_NTS::ModerateTheTeam() {
	...
	std::unique_lock<std::mutex> lock(arriving_incorporating_peers_mutex_);
	if (!Common_NTS::Contains(this->arriving_peers_, peer_id)) {
	...
	if (peer_info.source_port_to_splitter_ != 0
	    && !Common_NTS::Contains(peer_info.source_ports_to_monitors_, 0)) {
	  // Source ports are known, incorporate the peer
	  this->IncorporatePeer(peer_id);
	}

      } else if (std::find(this->peer_list_.begin(),
			   this->peer_list_.begin() + this->number_of_monitors_, sender)
		 != this->peer_list_.begin() + this->number_of_monitors_
	...
	uint16_t source_port_to_monitor = Common_NTS::Receive<uint16_t>(msg_str);

	// Get monitor number
	uint16_t index = Common_NTS::Index(this->peer_list_, sender);
	// Update peer information
	...
	if (peer_info.source_port_to_splitter_ != 0
      ...
      } else if (message.size() == Common_NTS::kPeerIdLength + 2) {
	...
	uint16_t source_port = Common_NTS::Receive<uint16_t>(msg_str);

	// Update source port information
	this->UpdatePortStep(*peer, source_port);

      } else if (message.size() == Common_NTS::kPeerIdLength + 1) {
	...
	std::unique_lock<std::mutex> lock(arriving_incorporating_peers_mutex_);
	if (!Common_NTS::Contains(this->incorporating_peers_, peer_id)) {
