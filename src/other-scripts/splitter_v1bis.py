...
parser.add_argument('--source_host', help='The streaming server. (Default = "{}")'.format(source_host))

source_port = 4551
parser.add_argument('--source_port', help='Port where the streaming server is listening. (Default = {})'.format(source_port))

parser.add_argument('--channel', help='Name of the channel served by the streaming source. (Default = "{}")'.format(channel))

listening_host = "127.0.0.1"
parser.add_argument('--listening_host', help='IP address to talk with the peers. (Default = {})'.format(listening_host))

parser.add_argument('--listening_port', help='Port to talk with the peers. (Default = {})'.format(listening_port))

parser.add_argument('--buffer_size', help='size of the video buffer in blocks. (Default = {})'.format(buffer_size))

parser.add_argument('--chunk_size', help='Chunk size in bytes. (Default = {})'.format(chunk_size))
...
class Splitter():
    ...
    # latency experimented by the users, and should be transmitted to
